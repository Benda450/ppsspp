...
int main(int argc, char **argv) {
	...
	string image_name = string(atlas_name) + "_atlas_canary.zim";
	string meta_name = string(atlas_name) + "_atlas_canary.meta";
	out_prefix = argv[2];
	...
	printf("Done. Outputting source and meta files %s_atlas_canary.cpp/h/meta.\n", out_prefix.c_str());
	// Sort items by ID.
	...
	fprintf(cpp_file, "const Atlas %s_atlas_canary = {\n", atlas_name);
	fprintf(cpp_file, "  \"%s\",\n", image_name.c_str());
	if (fonts.size()) {
	...
	FILE *h_file = fopen((out_prefix + "_atlas_canary.h").c_str(), "wb");
	fprintf(h_file, "// Header generated by atlastool from %s (hrydgard@gmail.com)\n\n", argv[1]);
	fprintf(h_file, "#pragma once\n");
	fprintf(h_file, "#include \"gfx/texture_atlas_canary.h\"\n\n");
	if (fonts.size()) {
	...
	fprintf(h_file, "extern const Atlas %s_atlas_canary;\n", atlas_name);
