...
public:
	ComboKey(uint64_t pspButtonBit, const char *key, bool toggle, EmuScreen* emuScreen, ImageID bgImg, ImageID bgDownImg, ImageID img, float scale, UI::LayoutParams *layoutParams)
		: MultiTouchButton(key, bgImg, bgDownImg, img, scale, layoutParams), pspButtonBit_(pspButtonBit), toggle_(toggle), emuScreen_(emuScreen), on_(false) {
	}
	void Touch(const TouchInput &input) override;
	bool IsDown() override;
private:
